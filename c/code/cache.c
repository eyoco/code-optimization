...
static int cache_line_retrieve_data(cache_line_t *cache_line, size_t offset)
{
	...
	while(count < 4){
		data += cache_line -> data[offset + count] << (8 * count);
		count = count + 1;
	}
...
static cache_line_t *cache_set_find_matching_line(cache_t *cache, cache_set_t *cache_set, intptr_t tag)
{
     ...
     */
	 ...
	 while(count < cache -> associativity){
		 if(cache_line_check_validity_and_tag(cache_set -> cache_lines[count], tag)){
			 return cache_line_make_mru(cache_set, count);
		 }
		 count = count + 1;
	 }
...
static cache_line_t *find_available_cache_line(cache_t *cache, cache_set_t *cache_set)
{
	...
	while(count < cache -> associativity){
		if(cache_set -> cache_lines[count] -> is_valid){
			return cache_line_make_mru(cache_set, count);
		}
		count = count + 1;
	}
...
int cache_read(cache_t *cache, int *address)
{
	...
	cache_line_t * cacheLINE = cache_set_find_matching_line(cache, &(cache->sets[((uintptr_t)address >> cache->cache_index_shift) & cache->cache_index_mask]), (uintptr_t)address >> cache->tag_shift);
	cache -> access_count = cache -> access_count + 1;
	if(cacheLINE == NULL || cacheLINE -> tag != ((uintptr_t)address >> cache->tag_shift) || cacheLINE -> is_valid == 0){
		cacheLINE = cache_set_add(cache,  &(cache->sets[((uintptr_t)address >> cache->cache_index_shift) & cache->cache_index_mask]), (uintptr_t)address, (uintptr_t)address >> cache->tag_shift);
		cache -> miss_count = cache -> miss_count + 1;
	}	
	return cache_line_retrieve_data(cacheLINE, (uintptr_t)address & cache->block_offset_mask);	
}
